...
export async function uploadExcelAction(formData: FormData) {
    ...
    try {
        ...
        // v15.4: 중복된 전화번호가 있을 경우, 더 정보량이 많은(긴) 데이터를 우선순위로 병합하는 스마트 엔진 도입
            ...
            if (phone) {
                ...
                if (cleanPhone.length >= 9) {
                    ...
                    if (!existing) {
                        ...
                        // 스마트 병합: 기존 데이터와 비교하여 더 나은(긴) 정보를 선택
                        ...
                        (Object.keys(currentData) as Array<keyof typeof currentData>).forEach(key => {
                            ...
                            // 새 데이터가 더 길고, 점세개(...)를 포함하지 않으면 업데이트
                            // 단, 기존 데이터가 점세개를 포함하고 있다면 새 데이터가 짧더라도 업데이트 (복구)
        ...
        // DB에 구형 포맷으로 저장된 유저들이 예기치 않게 삭제되는 것을 방지합니다.
